...
DEFAULT_BEND_UP_CENTS = DEFAULT_WOW_DEPTH
DEFAULT_BEND_DOWN_CENTS = DEFAULT_WOW_DEPTH
...
def generate_wobble_data(options: dict) -> List[tuple[float, int]]:
    """
...
class TapeWobbleEffect(MidiEffect):
    ...
    def _process_sequence_impl(self, 
        ...
        # Convert legacy events if needed and ensure all events have integer ticks
        normalized_events = []
        max_tick = 0
        ...
        for event in events:
            ...
            # Normalize the tick value to an integer
            try:
                tick = int(float(event[1])) if isinstance(event[1], (int, float, str)) else 0
                max_tick = max(max_tick, tick)
                # Create new event tuple with integer tick
                normalized_event = (event[0], tick) + event[2:]
        ...
        result_events: List[MidiInstruction] = []
        
        # First, add the initial pitch bend center with integer tick
        result_events.append(('pitch_bend', 0, MIDI_PITCH_BEND_CENTER, 0, 0))
        ...
        for time_sec, bend_value in wobble_data:
            ...
            if abs(bend_value - MIDI_PITCH_BEND_CENTER) > PITCH_BEND_THRESHOLD:
                result_events.append(('pitch_bend', tick, bend_value, 0, 0))
        
        # Add final pitch bend center at the end
        result_events.append(('pitch_bend', max_tick, MIDI_PITCH_BEND_CENTER, 0, 0))
            
        # Sort by tick time, with specific ordering for simultaneous events
        def event_sort_key(event):
            if not isinstance(event, tuple) or len(event) < 2:
                return (0, 1)  # Handle unexpected event format
            
            # All ticks should be integers at this point
            tick = event[1]  # Already normalized to int above
            
            # Determine event priority (pitch_bend: 0, note_on: 1, note_off: 2)
            msg_type = str(event[0])
        ...
        return sorted(result_events, key=event_sort_key)
