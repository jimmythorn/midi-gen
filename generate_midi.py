...
def generate_midi_with_options(options):
    ...
    timing_variance = options.get('timing_variance', 10)  # Lower variance for more consistency
    swing_strength = options.get('swing_strength', 0.4)  # Strength of groove
    velocity_variance = options.get('velocity_variance', 5)  # Lower variance in note velocity
    ...
    def arpeggiate_with_melody(notes, scale):
        ...
        for note in arpeggio:
            if random.random() < 0.2:  # Less randomness for consistency
